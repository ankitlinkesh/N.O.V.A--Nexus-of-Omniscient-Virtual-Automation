...
def maybe_handle_fast_command(
    ...
    # Phase 96 agent-driven GUI control. Console-only, because opening the scope
    # IS the authorization: inside it the planner can see and choose `screen.*`,
    # and nothing untrusted may ever make that choice. `gui:` with a colon cannot
    # begin ordinary prose, so this cannot start swallowing ordinary requests.
